/* GAEPP — Pruefstand: der Rechenkern.
   ...
   f) Die Knoepfe `[data-nullen]` und `[data-alle-um]`. In der laufenden App
      ...
      Ihre Arbeit tun die Tasten `n` und `z`. Dass diese zwei Tasten irgendwo
      genannt werden, ist eine neue Pruefung — zuerst an der Blattlegende der
      dritten Kopfzeile, seit deren Streichung am 23.08.2026 am Handbuch.
...
async function frisch() {
  ...
  await ruhe();
  vorratZuruecksetzen();
  /* Die Tasten wirken nur ausserhalb eines Eingabefelds — darum vorher auf ein
     Stueck Kopfzeile klicken, das keines ist. Bis zum 23.08.2026 war das der
     Blatttitel (.titel); die dritte Kopfzeile ist gestrichen. An ihre Stelle
     tritt die Wortmarke — kein Feld, kein Knopf, nichts wird dabei geprueft,
     der Klick holt nur den Fokus aus dem Blatt. */
  await seite.click('#leiste .wort');
  if (!(await seite.evaluate(() => S.nullen))) await taste('n');
  for (let i = 0; i < 3; i++) {
    if (await zaehl('tr.pos')) break;
    await seite.click('#leiste .wort'); await taste('z'); await seite.waitForTimeout(250);
  }
}
...
/* Die beiden Knoepfe von frueher sind weg — an ihrer Stelle stehen zwei Tasten.
   Gemessen wird der Text, nicht das Vorhandensein einer Regel im Blatt. */
...
/* GESTRICHEN am 23.08.2026: die Pruefung an `.kopf3 .legende`. Die dritte
   Kopfzeile — Blatttitel links, Legende rechts — ist ersatzlos entfallen; es
   gibt kein Feld mehr, an dem sich diese Pruefung stellen liesse. Die Frage
   dahinter lebt weiter: erfaehrt der Bediener ueberhaupt, dass es diese zwei
   Tasten gibt? Sie wird deshalb am neuen Ort weitergestellt — im Handbuch,
   Abschnitt «Tastatur». Dazu die Gegenprobe, dass das Fenster danach wieder
   zu ist und der Lauf nicht in einem offenen Dialog weiterrechnet. */
await klick('[data-hb]');
const handbuch = await text('[data-schleier="hb"]');
pruef('das Handbuch nennt beide Tasten', handbuch !== null
  && /z\s+klappt\s+alles\s+zu/.test(handbuch)
  && /n\s+zeigt\s+die\s+Nullen/.test(handbuch), handbuch);
pruef('und sagt dazu, dass sie im Eingabefeld nicht wirken',
  handbuch !== null && /wirken\s+nicht,\s+w[äa]hrend\s+du\s+in\s+einem\s+Feld\s+tippst/.test(handbuch),
  handbuch);
await taste('Escape');
gleich('Gegenprobe: das Handbuch ist danach wieder zu',
  await zaehl('[data-schleier="hb"]'), 0);
gleich('Gegenprobe: es steht ueberhaupt kein Fenster mehr offen',
  await zaehl('.schleier'), 0);
...
if (pidZulagen) {
  ...
  await seite.click('#leiste .wort'); await taste('n');
