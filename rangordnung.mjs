/* GAEPP — Pruefstand: die Rangordnung.
   ...
   Nachgefuehrt am 23.08.2026: Der Kopf hat nur noch zwei Zeilen. Die dritte,
   die den Blatttitel und die Legende trug, ist ersatzlos gestrichen — und mit
   ihr der Rang R2, den die Uebergabe fuer den Blatttitel vorsah. Die Messungen
   dieser Zeile sind damit gegenstandslos und stehen unten als Streichung mit
   Begruendung, an ihrer Stelle steht nichts Neues. Wo dieser Lauf den Titel
   nicht gemessen, sondern nur gelesen hat, um zu wissen, welches Blatt dasteht,
   liest er jetzt den Ankerpunkt: den hellen Jahrgang und das helle
   Ansichtswort. Der Klick, der den Fokus aus einem Feld holt, geht auf die
   Wortmarke. */
...
/* ---- Der Ankerpunkt: woran dieser Lauf erkennt, welches Blatt dasteht ----

   Bis zum 23.08.2026 trug eine dritte Kopfzeile den Blatttitel («Budget 2026»).
   Daran hat dieser Lauf abgelesen, worauf er gerade misst, und darauf gewartet.
   Die Zeile ist gestrichen; dieselbe Auskunft steht eine Zeile hoeher und ist
   dort die Wirkung, die der Benutzer liest:
     - der helle Jahrgang   .jg.an   (die Jahreszahl),
     - das helle Ansichtswort .ans.an (BUDGET / RECHNUNGEN / ALLE JAHRE).
   In «Alle Jahre» ist kein Jahrgang hell — es sind ja alle gemeint — und es
   sind zwei Woerter hell: «Alle Jahre» und der Zweig, auf den man schaut.

   Gelesen wird der sichtbare Text: die Versalschreibung kommt aus der Gestaltung
   (text-transform), also wird sie mitgerechnet und nicht der rohe Knopftext
   genommen. Fehlt der Jahrgang, steht ein Strich — so ist «kein Jahrgang hell»
   von «Jahrgang nicht gefunden» im Ergebnis nicht zu verwechseln, denn ohne die
   Kopfzeile stuende dort «— · —».
   Die Reihenfolge der Woerter ist die des Blattes, von links nach rechts. */
const ankerLesen = () => {
  const sicht = (e) => {
...
const anker = () => seite.evaluate(ankerLesen);

/* Die drei Bilder, gegen die dieser Lauf wartet — einmal benannt statt
   viermal buchstabiert. Sie fassen die Stelle im Ankerpunkt, an der das
   jeweilige Blatt eindeutig wird: in einem Jahrgang steht das Ansichtswort
   allein am Ende, in «Alle Jahre» steht «ALLE JAHRE» zuletzt und davor der
   Zweig. */
const A_BUDGET   = '· BUDGET$';
const A_RECHNUNG = '· RECHNUNGEN$';
const A_ALLE     = 'ALLE JAHRE$';

/* Warten, bis das Blatt wirklich das ist, auf das gemessen werden soll. Ein
   fester Zeitwert waere geraten; der Ankerpunkt ist gemessen. Gewartet wird
   ueber denselben Leser wie oben — zwei Rechnungen waeren zwei Wahrheiten. */
const warteAnker = async (muster, ms) => {
  const re = new RegExp(muster);
  const bis = Date.now() + (ms || 8000);
  let ist = await anker();
  while (!re.test(ist)) {
    if (Date.now() > bis)
      throw new Error('Ankerpunkt bleibt «' + ist + '», erwartet wurde ' + muster);
...
/* Aus «Alle Jahre» heraus schaltet ein Klick auf ein Ansichtswort nur den Zweig
   ...
   Der Jahrgangsknopf wird ueber den Waehler geklickt und nicht ueber einen
   vorher geholten Knotenverweis: jede Bedienung zeichnet den Kopf neu, ein
   gehaltener Verweis zeigt danach auf einen Knoten, der nicht mehr im Blatt
   haengt. Beim Suchen darf kurz gewartet werden, am Schluss voll. */
const geh = async (wahl, muster) => {
  for (let i = 0; i < 4; i++) {
    await seite.click(wahl);
    try { await warteAnker(muster, 1200); return; } catch (e) { /* weiter unten */ }
    if (await zaehl('.kopf2 .jg')) {
      await seite.click('.kopf2 .jg');
      await seite.waitForTimeout(300);
    }
  }
  await warteAnker(muster);
};

/* Im Ruhezustand koennen Sektionen zu sein — dann gibt es keine Gruppen- und
   Positionszeilen zu messen. «z» klappt alles auf. Die Taste wirkt nicht,
   waehrend in einem Feld getippt wird; frueher holte ein Klick auf den
   Blatttitel den Fokus heraus. Den gibt es nicht mehr — dafuer steht jetzt die
   Wortmarke da: ein Wort ohne Bedienung, das den Fokus nimmt, ohne nebenbei
   etwas zu schalten. */
const aufklappen = async () => {
  for (let i = 0; i < 3; i++) {
    if (await zaehl('tr.pos')) break;
    await seite.click('.wort');
...
/* =========================================================================
   ...
   ========================================================================= */
await geh('[data-geh-ansicht="budget"]', A_BUDGET);
...
/* =========================================================================
   ...
   ungefaehr).
   Der Kopf hat seit dem 23.08.2026 nur noch zwei Zeilen; was zur dritten
   gehoerte, steht unten bei den gestrichenen Messungen.
   ========================================================================= */
...
/* GESTRICHEN am 23.08.2026 — die dritte Kopfzeile und mit ihr der Rang R2.
   Hier stand die Messung des Blatttitels (Groesse 22, Gewicht 300, Text
   «Budget» und Jahreszahl) und daneben die der Legende (11.5/300, drei Teile,
   Haken, Stichmonat, die beiden Tasten). Beides hing an .kopf3 mit .titel und
   .legende. Die Zeile ist an diesem Tag ersatzlos entfallen: der Titel sagte
   dasselbe, was eine Zeile darueber schon zweimal steht — als heller Jahrgang
   und als helles Ansichtswort —, und die Legende steht im Handbuch.

   Diese Messungen sind damit nicht falsch geworden, sondern gegenstandslos:
   es gibt kein Element mehr, an dem sie ansetzen koennten. Sie werden nicht
   umgehaengt, denn der Rang R2 kommt im Werkzeug nicht mehr vor — es gibt
   nichts, was seine Stelle einnimmt. Nur eine Pruefung, die sonst gemessen
   haette, waere hier zu ersetzen; eine Zeile, die es nicht mehr gibt, ist
   nicht zu ersetzen.

   Was von der Frage «wo stehe ich?» bleibt, misst dieser Lauf weiter — oben
   an .jg.an und .ans.an und in den Abschnitten 3 und 4 am Ankerpunkt.
   Wichtig: Nichts an dieser Streichung ist eine Lockerung. Keine Erwartung ist
   herabgesetzt worden, kein Rot ist weggenommen worden, das die App betrifft. */
...
pruef('Fusszeile rechts nennt Jahrgaenge, Haken und «alles gerechnet»',
  ...
  rechts.text);
/* GESTRICHEN am 23.08.2026, aus demselben Grund wie oben: Hier stand
   «kein Untertitel neben dem Blatttitel» — gezaehlt wurden die zwei Kinder von
   .kopf3 (Titel und Legende). Mit der dritten Kopfzeile faellt auch die Frage
   weg, was neben dem Titel steht. Dass der Anspruch «Passend | Präzise |
   Praktisch» nur in der Fusszeile steht, prueft die Messung darueber weiterhin
   — sie ist der Teil dieser Aussage, der einen Gegenstand behalten hat. */
...
/* =========================================================================
   ...
   ========================================================================= */
await geh('[data-geh-ansicht="rechnung"]', A_RECHNUNG);
await zumJahr(STICHJAHR);
console.log('\n4 — Rechnungen');
gleich('Ankerpunkt', await anker(), STICHJAHR + ' · RECHNUNGEN');
...
await warteAnker(A_ALLE);
console.log('\n3 — Alle Jahre');
gleich('Ankerpunkt', await anker(), '— · BUDGET · ALLE JAHRE');
