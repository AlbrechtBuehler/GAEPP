/* GAEPP — Pruefstand: der Rahmen.
   ...
   23.08.2026 — die dritte Kopfzeile ist gestrichen (Blatttitel links, Legende
   rechts), und mit ihr das Feld #blattkopf und die Klassen .kopf3, .titel und
   .legende. Fuenf Pruefungen dieses Laufs hatten sie zum Gegenstand. Sie sind
   nachgezogen, nicht weggeworfen: zwei sind auf die neue Zahl der Felder
   umgestellt (sieben statt acht, vier statt fuenf im Rollfeld), drei sind
   gestrichen und tragen ihre Begruendung an Ort und Stelle. An ihre Stelle
   tritt der Abschnitt «Die gestrichene Zeile drei» — er prueft, dass die
   Auskunft der Zeile nicht verloren ist, sondern gewandert: hell im Jahrgang,
   hell im Ansichtswort, woertlich im Kopf von Papier und Exportdatei. */
...
/* NACHGEZOGEN am 23.08.2026. Bis dahin waren es acht Felder, fuenf davon im
   Rollfeld. Mit der dritten Kopfzeile ist auch ihr Traeger #blattkopf
   ersatzlos entfallen: es sind sieben Felder, vier davon rollen. Geprueft wird
   weiter die Folge im Dokument UND die Zugehoerigkeit — die alte Fassung las
   nur die unmittelbaren Kinder des Koerpers und wuerde jede Schachtelung fuer
   einen Verlust halten. */
const RAHMENFELDER = '#rollfeld,#leiste,#band,#druckkopf,#blatt,#druckfuss,#fusszeile,#dialoge';
const holeFolge = w => seite.evaluate(s => [...document.querySelectorAll(s)].map(e => e.id), w);
const rahmenFolge = await holeFolge(RAHMENFELDER);
gleich('die sieben Felder des Rahmens stehen in dieser Folge',
  rahmenFolge.filter(x => x !== 'rollfeld').join(' '),
  'leiste band druckkopf blatt druckfuss fusszeile dialoge');
/* Gegenprobe zur verkuerzten Liste. Dieselbe Suche, um #blattkopf erweitert,
   bringt kein Feld dazu — die Liste ist also kurz, weil das Feld weg ist, und
   nicht, weil es beim Nachziehen bloss aus der Suche gefallen waere. */
const rahmenMitAlt = await holeFolge(RAHMENFELDER + ',#blattkopf');
gleich('Gegenprobe: mit #blattkopf gesucht kommt kein Feld dazu',
  rahmenMitAlt.join(' '), rahmenFolge.join(' '));
const rollKinder = await seite.evaluate(() => [...document.querySelector('#rollfeld').children]
  .filter(e => e.id).map(e => e.id));
gleich('vier davon liegen im Rollfeld und rollen gemeinsam',
  rollKinder.join(' '), 'leiste band druckkopf blatt');
...
/* ------------------------------------------------ Das Rollfeld (neu) ------ *
   ...
   liessen elf Datenzeilen uebrig. Seither rollen Wortmarke und Band weg;
   stehen bleiben Jahrgang und Ansicht, darunter die Spaltenkoepfe, darunter
   die Saldozeile. Gemessen wird die Wirkung: wo die Kanten nach dem Rollen
   liegen — nicht, welche Regel gesetzt ist.
   NACHGEZOGEN am 23.08.2026: Der dritte Kopfbaustein, die Titelzeile, ist
   gestrichen. Er kann nicht mehr wegrollen, weil er nicht mehr dasteht. */
...
const kante = () => seite.evaluate(() => {
  ...
  return { rollTop: roll.scrollTop, roller: Math.round(roll.scrollHeight) > Math.round(roll.clientHeight),
    wort: o('.kopf1'), kopf2: o('.kopf2'), band: o('#band'),
...
/* GESTRICHEN am 23.08.2026: «der Blatttitel rollt weg». Die Zeile, die er trug,
   gibt es nicht mehr; eine Kante, die es nicht gibt, kann nicht wandern. Die
   Pruefung ist damit gegenstandslos geworden — nicht falsch, sondern leer.
   Ersatz steht im Abschnitt «Die gestrichene Zeile drei» weiter unten: dort
   wird geprueft, wo die Auskunft des Titels geblieben ist. */
...
/* GESTRICHEN am 23.08.2026: «der Blattkopf traegt Zeile drei». Weder das Feld
   noch die Zeile gibt es noch — die Pruefung haette keinen Gegenstand mehr.
   Sie wird nicht stillschweigend entfernt: an ihre Stelle tritt der Abschnitt
   «Die gestrichene Zeile drei» unmittelbar darunter. Eine gestrichene Pruefung
   ist eine Luecke, solange nicht geprueft ist, wo ihr Gegenstand geblieben
   ist — die Zeile ist weg, ihre Auskunft ist gewandert. */
gleich('das Band steht zwischen Leiste und Blatt', await zaehl('#band .band'), 1);
gleich('kein Dialog offen, also ist das Dialogfeld leer',
  await seite.evaluate(() => document.getElementById('dialoge').innerHTML.trim()), '');

/* ================================= Die gestrichene Zeile drei ============= */
/* Am 23.08.2026 ist die dritte Kopfzeile gestrichen worden — links der
   Blatttitel («Budget 2026»), rechts die Legende. Drei Pruefungen dieses Laufs
   hatten sie zum Gegenstand und sind entfallen; die Stellen tragen je eine
   Begruendung. Hier steht, was an ihre Stelle tritt.
   Gestrichen ist die ZEILE, nicht ihre Auskunft. Wovon der Titel sprach, steht
   weiterhin an drei Stellen: hell im Jahrgang der Kopfzeile zwei, hell im
   Ansichtswort daneben, und woertlich im Kopf von Papier und Exportdatei.
   Geprueft wird die Wirkung: sichtbarer Text und berechnete Auszeichnung, nie
   die Regel, die sie erzeugt. Und geprueft wird in allen vier Ansichten, denn
   ein Titel, der nur im Budget gewandert waere, waere anderswo verloren. */
console.log('\nDie gestrichene Zeile drei — die Auskunft ist gewandert, nicht weg');

const auskunft = () => seite.evaluate(() => {
  const txt = e => (e.textContent || '').trim();
  const liste = w => [...document.querySelectorAll(w)].map(txt);
  const messe = w => {
    const e = document.querySelector(w); if (!e) return null;
    const c = getComputedStyle(e);
    return { text: txt(e), farbe: c.color, gewicht: Number(c.fontWeight) };
  };
  const dk = document.querySelector('#druckkopf .dkTitel');
  return {
    jgHell: liste('.jg.an'), ansHell: liste('.ans.an'),
    jgLeise: liste('.jg:not(.an)'), ansLeise: liste('.ans:not(.an)'),
    einJgHell: messe('.jg.an'), einJgLeise: messe('.jg:not(.an)'),
    einAnsHell: messe('.ans.an'), einAnsLeise: messe('.ans:not(.an)'),
    druckTitel: dk ? txt(dk) : null
  };
});

/* Die drei Klassen der gestrichenen Zeile kommen im Dokument nirgends mehr
   vor — weder als Knoten noch als Wahl im Stilblatt. Gesucht wird mit genauem
   Klassennamen und Grenze dahinter: «.dtitel» (Dialogkopf) und «.dkTitel»
   (Titel im Druckkopf) sind andere Klassen und duerfen nicht mittreffen. */
const verschwunden = () => seite.evaluate(() => {
  const namen = ['kopf3', 'titel', 'legende'];
  const genau = n => new RegExp('\\.' + n + '(?![\\w-])');
  const wahlen = [];
  const sammle = liste => [...liste].forEach(r => {
    if (r.selectorText) wahlen.push(r.selectorText);
    if (r.cssRules) sammle(r.cssRules);
  });
  [...document.styleSheets].forEach(bl => { try { sammle(bl.cssRules); } catch (e) { /* fremd */ } });
  const knoten = {}, regeln = {};
  namen.forEach(n => {
    knoten[n] = document.querySelectorAll('.' + n).length;
    regeln[n] = wahlen.filter(w => genau(n).test(w)).length;
  });
  return { knoten, regeln,
    alleKnoten: document.querySelectorAll('*').length, alleWahlen: wahlen.length,
    probeKnoten: document.querySelectorAll('.kopf2').length,
    probeRegeln: wahlen.filter(w => /\.kopf2(?![\w-])/.test(w)).length,
    probeDialog: document.querySelectorAll('.dtitel, .dkTitel').length };
});

const wandert = async (setze, label, sollTitel) => {
  await stand(setze, STICHJAHR);
  const a = await auskunft();
  /* 1. Papier und Export tragen den Titel weiter — er lebt, nur nicht im Fenster. */
  gleich('Papier traegt den Blatttitel weiter — ' + label, a.druckTitel, sollTitel);
  /* 2. Jedes Wort des Titels steht hell in der Kopfzeile. Das ist der Ersatz
        fuer die gestrichene Pruefung: nicht, dass die Zeile fehlt, sondern
        dass ihre Auskunft noch abzulesen ist. */
  const helleWorte = a.jgHell.concat(a.ansHell).join(' ').split(/\s+/).filter(Boolean);
  const titelWorte = sollTitel.split(/[\s·]+/).filter(Boolean);
  pruef('jedes Wort des Blatttitels steht hell in der Kopfzeile — ' + label,
    titelWorte.length > 0 && titelWorte.every(w => helleWorte.indexOf(w) >= 0),
    'hell: ' + helleWorte.join(' ') + '  |  Titel: ' + sollTitel);
  /* 3. Gegenprobe: «hell» ist ein Unterschied, kein Wort. Es muss leise
        Nachbarn geben, sonst sagt die Auszeichnung nichts. */
  pruef('Gegenprobe: neben den hellen stehen leise Woerter — ' + label,
    a.jgLeise.length > 0 && a.ansLeise.length > 0,
    a.jgLeise.length + ' leise Jahrgaenge, ' + a.ansLeise.length + ' leise Ansichtsworte');
  /* 4. Und der Unterschied ist gemessen, nicht behauptet: heller und schwerer. */
  if (a.einJgHell) {
    pruef('der Jahrgang steht heller und schwerer als seine Nachbarn — ' + label,
      helligkeit(a.einJgHell.farbe) > helligkeit(a.einJgLeise.farbe)
      && a.einJgHell.gewicht > a.einJgLeise.gewicht,
      a.einJgHell.farbe + '/' + a.einJgHell.gewicht + ' gegen '
      + a.einJgLeise.farbe + '/' + a.einJgLeise.gewicht);
  } else {
    /* In «Alle Jahre» ist kein einzelner Jahrgang hell — das ist richtig so,
       der Titel spricht dort auch von keinem. Die Auskunft traegt allein das
       Ansichtswort, und genau das prueft Nummer 2. */
    gleich('in «Alle Jahre» ist kein einzelner Jahrgang hell — ' + label, a.jgHell.length, 0);
  }
  pruef('das Ansichtswort steht heller und schwerer als seine Nachbarn — ' + label,
    helligkeit(a.einAnsHell.farbe) > helligkeit(a.einAnsLeise.farbe)
    && a.einAnsHell.gewicht > a.einAnsLeise.gewicht,
    a.einAnsHell.farbe + '/' + a.einAnsHell.gewicht + ' gegen '
    + a.einAnsLeise.farbe + '/' + a.einAnsLeise.gewicht);
  /* 5. Und die gestrichenen Klassen kommen in dieser Ansicht nirgends vor. */
  const v = await verschwunden();
  gleich('.kopf3 kommt als Knoten nicht mehr vor — ' + label, v.knoten.kopf3, 0);
  gleich('.titel kommt als Knoten nicht mehr vor — ' + label, v.knoten.titel, 0);
  gleich('.legende kommt als Knoten nicht mehr vor — ' + label, v.knoten.legende, 0);
  pruef('Gegenprobe: die Suche hat ueberhaupt Knoten angesehen — ' + label,
    v.alleKnoten > 100 && v.probeKnoten === 1,
    v.alleKnoten + ' Knoten, davon .kopf2: ' + v.probeKnoten);
  return v;
};

const vBudget = await wandert(j => { S.ansicht = 'budget'; S.jahr = j; zeichne(); },
  'Budget', 'Budget ' + STICHJAHR);
await wandert(j => { S.ansicht = 'rechnung'; S.jahr = j; zeichne(); },
  'Rechnungen', 'Rechnungen ' + STICHJAHR);
await wandert(() => { S.ansicht = 'alle'; S.alleWas = 'budget'; zeichne(); },
  'Alle Jahre (Budget)', 'Alle Jahre');
await wandert(() => { S.ansicht = 'alle'; S.alleWas = 'rechnung'; zeichne(); },
  'Alle Jahre (Rechnungen)', 'Alle Jahre · Rechnungen');

/* Die Wahlen des Stilblatts sind nicht an eine Ansicht gebunden; sie werden
   einmal geprueft. Eine Regel fuer eine Klasse, die es nicht mehr gibt, waere
   kein Fehler in der Anzeige — aber ein Rest, der beim naechsten Umbau wieder
   mitgeschleppt wird. */
gleich('keine Regel im Stilblatt waehlt noch .kopf3', vBudget.regeln.kopf3, 0);
gleich('keine Regel im Stilblatt waehlt noch .titel', vBudget.regeln.titel, 0);
gleich('keine Regel im Stilblatt waehlt noch .legende', vBudget.regeln.legende, 0);
pruef('Gegenprobe: die Suche hat ueberhaupt Wahlen angesehen',
  vBudget.alleWahlen > 100 && vBudget.probeRegeln > 0,
  vBudget.alleWahlen + ' Wahlen, davon mit .kopf2: ' + vBudget.probeRegeln);
/* Gegenprobe zur Wortgrenze: «.dtitel» und «.dkTitel» gibt es weiter. Traefe
   die Suche nach «.titel» auch sie, waere das Gruen oben zufaellig. */
pruef('Gegenprobe: .dtitel und .dkTitel gibt es weiter und sie treffen nicht mit',
  vBudget.probeDialog > 0, vBudget.probeDialog + ' Knoten');

/* Die Luft, die die Titelzeile lieferte, kommt jetzt vom Blatt selbst. Gemessen
   wird der Abstand zwischen der Oberkante des Blatts und der ersten Zeile der
   Tabelle — nicht die Regel, die ihn setzt. Ohne diese Luft klebte die Tabelle
   unmittelbar unter der Kopfzeile. */
await stand(j => { S.ansicht = 'budget'; S.jahr = j; zeichne(); }, STICHJAHR);
await seite.evaluate(() => { document.querySelector('#rollfeld').scrollTop = 0; });
await ruhe();
const luft = await seite.evaluate(() => {
  const bl = document.querySelector('#blatt'), tab = bl.querySelector('table');
  if (!tab) return null;
  const b = bl.getBoundingClientRect(), t = tab.getBoundingClientRect();
  return { oben: Math.round(t.top - b.top), breite: Math.round(t.width) };
});
pruef('das Blatt traegt oben Luft, die Tabelle klebt nicht an der Kopfzeile',
  luft && luft.oben >= 16, luft && luft.oben + ' px');
pruef('Gegenprobe: gemessen wurde an einer Tabelle, die wirklich dasteht',
  luft && luft.breite > 200, luft && luft.breite + ' px breit');
await stand(j => { S.ansicht = 'budget'; S.jahr = j; zeichne(); }, STICHJAHR);
...
/* GESTRICHEN am 23.08.2026: «Kopfzeile drei ist im Druck ausgeblendet». Es gibt
   sie am Bildschirm nicht mehr, also auch nichts, was der Druck ausblenden
   koennte; die Pruefung suchte einen Knoten und fand «fehlt». Was sie sichern
   sollte — dass der Blatttitel nicht ins Papier durchschlaegt und dort doppelt
   steht —, sichern jetzt zwei Pruefungen zusammen: «Papier traegt den
   Blatttitel weiter» im Abschnitt «Die gestrichene Zeile drei» und die Zeile
   direkt darunter, die zaehlt, wie oft der Titel auf dem Papier vorkommt. */
...
/* Der Blatttitel steht auf dem Papier genau einmal — im Druckkopf. Das ist der
   Ersatz fuer die gestrichene Zeile «Kopfzeile drei ist im Druck
   ausgeblendet»: geprueft wird nicht mehr, dass etwas verschwindet, sondern
   dass es an der einen richtigen Stelle steht und nirgends sonst. */
const titelAufPapier = await seite.evaluate(() => {
  const sichtbar = e => { const c = getComputedStyle(e);
    return c.display !== 'none' && c.visibility !== 'hidden'; };
  return [...document.querySelectorAll('.dkTitel')].filter(e =>
    sichtbar(e) && e.closest('#druckkopf')).length;
});
gleich('der Blatttitel steht auf dem Papier genau einmal, im Druckkopf', titelAufPapier, 1);
...
/* Und zwar im Kopf der Datei, in .dkTitel — nicht bloss irgendwo im Text.
   Seit die dritte Kopfzeile gestrichen ist, ist der Export neben dem Papier
   der zweite Ort, an dem der Blatttitel woertlich steht; darum wird hier die
   Stelle geprueft und nicht nur das Vorkommen. */
pruef('der Blatttitel steht im Kopf der Datei, in .dkTitel',
  exp && exp.text.indexOf('<span class="dkTitel">Budget ' + STICHJAHR + '</span>') >= 0);
