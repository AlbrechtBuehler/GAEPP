...
/* Wo stehe ich? Bis zum 23.08.2026 stand die Antwort im Blatttitel der dritten
   Kopfzeile. Die Zeile ist ersatzlos gestrichen — #blattkopf, .titel und
   .legende gibt es nicht mehr. Die sichtbare Antwort steht seither in der
   zweiten Kopfzeile: der helle Jahrgang (.jg.an) und das helle Ansichtswort
   (.ans.an). Gelesen wird genau das — die Wirkung, nicht der Zustand.
   In «Alle Jahre» ist kein Jahrgang hell und es sind zwei Woerter hell (die
   Ansicht und das, was sie zeigt); dafuer stehen der Strich und das Pluszeichen. */
const standLesen = () => seite.evaluate(() => {
  const j = document.querySelector('#leiste .kopf2 .jg.an');
  const w = Array.from(document.querySelectorAll('#leiste .kopf2 .ans.an'))
    .map(b => (b.textContent || '').trim());
  return (j ? (j.textContent || '').trim() : '—') + ' \u00b7 ' + (w.length ? w.join(' + ') : '—');
});
/* Der gemerkte Zustand daneben — als Gegenprobe zur Anzeige, nie an ihrer
   Stelle. Was hell steht, muss dasselbe sagen wie das, was die App sich merkt. */
const standZustand = () => seite.evaluate(() =>
  S.jahr + '/' + S.ansicht + '/' + S.alleWas);
...
/* ============================================================ 8. Jahrgänge */
{
  ...
  /* Wechsel zwischen Jahrgang und Ansicht. Seit die dritte Kopfzeile weg ist,
     sind der helle Jahrgang und das helle Ansichtswort die sichtbare Antwort
     darauf, worauf man gerade schaut. Neben jeder Messung steht der gemerkte
     Zustand als Gegenprobe — die Frage ist dieselbe geblieben, nur der
     Messpunkt ist gewandert. */
  await klick('[data-geh-jahr="' + jahreVor[0] + '"]');
  gleich('ein anderer Jahrgang', await standLesen(), jahreVor[0] + ' \u00b7 Budget');
  gleich('Gegenprobe im Zustand', await standZustand(), jahreVor[0] + '/budget/budget');
  await klick('[data-geh-ansicht="rechnung"]');
  gleich('die andere Ansicht', await standLesen(), jahreVor[0] + ' \u00b7 Rechnungen');
  gleich('Gegenprobe im Zustand', await standZustand(), jahreVor[0] + '/rechnung/budget');
  await klick('[data-geh-alle]');
  gleich('«Alle Jahre» behält, worauf man schaut',
    await standLesen(), '— \u00b7 Rechnungen + Alle Jahre');
  gleich('Gegenprobe im Zustand', await standZustand(), jahreVor[0] + '/alle/rechnung');
  await klick('[data-geh-ansicht="budget"]');
  gleich('und lässt sich dort umschalten', await standLesen(), '— \u00b7 Budget + Alle Jahre');
  gleich('Gegenprobe im Zustand', await standZustand(), jahreVor[0] + '/alle/budget');
  await klick('[data-geh-jahr="' + JAHR + '"]');
  gleich('zurück in einen Jahrgang führt in dieselbe Ansicht',
    await standLesen(), JAHR + ' \u00b7 Budget');
  gleich('Gegenprobe im Zustand', await standZustand(), JAHR + '/budget/budget');
}
...
/* ======================================================= 13. Notausgang (R13)
   ...
   Am 23.08.2026 ist die dritte Kopfzeile gestrichen; #blattkopf gibt es nicht
   mehr, und die Aufräumliste des Notausgangs führt nur noch band, druckkopf,
   druckfuss und dialoge. Die Frage von R13 bleibt und wird an ihrem neuen Ort
   gestellt: neben der Fehlermeldung darf kein Rest der alten Anzeige stehen —
   kein Blattkopf (er darf gar nicht mehr da sein), kein heller Jahrgang, kein
   Ansichtswort. Dazu die Gegenprobe, dass beides vorher wirklich dastand.
   ...
   Dieser Abschnitt steht zuletzt: danach ist die Seite absichtlich tot. */
{
  ...
  const ANSWORTE = ['Budget', 'Rechnungen', 'Alle Jahre'];
  const kopfText = () => seite.evaluate(() =>
    ['leiste', 'band', 'druckkopf', 'druckfuss', 'fusszeile']
      .map(id => (document.getElementById(id) || {}).textContent || '').join(' '));
  /* Gegenprobe zur Suche weiter unten: solange die App steht, stehen Jahrgang
     und alle drei Ansichtswörter wirklich im Kopf. Ohne sie suchte die
     Prüfung danach nach etwas, das es ohnehin nirgends gibt — und wäre
     grün, ganz gleich, was der Notausgang tut. */
  const jetztJahr = await seite.evaluate(() => String(S.jahr));
  const kopfVorher = await kopfText();
  pruef('Gegenprobe: vorher stehen Jahrgang und alle drei Ansichtswörter im Kopf',
    kopfVorher.indexOf(jetztJahr) >= 0 && ANSWORTE.every(w => kopfVorher.indexOf(w) >= 0),
    kopfVorher.slice(0, 90));
  const standVorher = await standLesen();
  pruef('Gegenprobe: vorher ist ein Jahrgang hell und mindestens ein Ansichtswort',
    standVorher.indexOf('—') < 0, standVorher);
  let getroffen = null;
  ...
  const not = await seite.evaluate(() => ({
    ...
    /* GESTRICHEN am 23.08.2026: das Feld blattkopf. #blattkopf gibt es in der
       App nicht mehr — getElementById lieferte null und der ganze Messgang
       brach ab. An seine Stelle tritt blattkopfDa: geprüft wird jetzt, dass
       es das Feld überhaupt nicht mehr gibt. */
    blattkopfDa: !!document.getElementById('blattkopf'),
    druckkopf: document.getElementById('druckkopf').innerHTML.trim(),
    druckfuss: document.getElementById('druckfuss').innerHTML.trim(),
    ...
    /* Der neue Ort, an dem stünde, auf welches Blatt sich die Meldung bezieht. */
    jgs: document.querySelectorAll('.jg').length,
    anss: document.querySelectorAll('.ans').length,
    kopfbereich: ['leiste', 'band', 'druckkopf', 'druckfuss', 'fusszeile']
      .map(id => (document.getElementById(id) || {}).textContent || '').join(' ')
  }));
  ...
  pruef('den Blattkopf gibt es gar nicht mehr — es bleibt nichts zu räumen',
    not.blattkopfDa === false, not.blattkopfDa);
  pruef('über der Meldung steht kein Jahrgang mehr',
    JAHRE.every(j => not.kopfbereich.indexOf(String(j)) < 0),
    not.kopfbereich.slice(0, 80));
  pruef('über der Meldung steht kein Ansichtswort mehr',
    ANSWORTE.every(w => not.kopfbereich.indexOf(w) < 0),
    not.kopfbereich.slice(0, 80));
  gleich('kein Jahrgangsknopf ist mehr da', not.jgs, 0);
  gleich('kein Ansichtsknopf ist mehr da', not.anss, 0);
  pruef('Band, Druckkopf, Druckfuss und Fusszeile sind geräumt',
    not.band === '' && not.druckkopf === '' && not.druckfuss === '' && not.fusszeile === '',
    'band «' + not.band.slice(0, 30) + '» druckkopf «' + not.druckkopf.slice(0, 30)
    + '» druckfuss «' + not.druckfuss.slice(0, 30) + '» fusszeile «'
    + not.fusszeile.slice(0, 30) + '»');
